...
- **Iced/wgpu version coupling**: the workspace `wgpu` dependency is pinned to version 27 because Iced 0.14's `iced_wgpu` backend uses wgpu 27. Do not independently bump `wgpu` until Iced supports the newer major version or the Iced app is updated in lockstep.
