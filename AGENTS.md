...
| `config_workspace.rs` | Shared session config workspace: tracks optional draft TOML, a last-applied `ConfigDocument`, and an optional bundled default document per entry; derives text/config views on demand and supports selection retention, custom entry copying, apply, revert, reset, and dirty detection for both UIs |
...
- **Session config workspace**: `ConfigWorkspace` is the shared source of config editor state for both UIs. Each entry stores only an optional dirty draft, a last-applied `ConfigDocument`, and an optional bundled default document; names, applied text, and `Config` values are derived from those documents on demand. Copy creates a renamed custom copy that preserves dirty draft text separately from the last-applied document, failed Apply keeps the current rendered scene, Revert drops the dirty draft, Reset restores bundled defaults for preset entries when the last-applied document differs from the default, custom entries have no bundled default, and dirty drafts disable config-affecting controls until Apply/Revert.
