...
| `config_workspace.rs` | Shared session config workspace: tracks optional draft TOML, a last-applied `ConfigDocument`, and an optional bundled default document per entry; derives text/config views on demand and exposes indexed copy/apply/revert/reset operations while each UI owns selection |
