...
| `config_workspace.rs` | Shared session config workspace: tracks one draft TOML text, last-applied TOML text, last-applied `Config`, and optional bundled default per entry; supports selection retention, apply, revert, reset, and dirty detection for both UIs |
...
- **Session config workspace**: `ConfigWorkspace` is the shared source of config editor state for both UIs. Each entry retains draft text independently from the last-applied document/config, failed Apply keeps the current rendered scene, Revert restores the last-applied text, Reset restores bundled defaults for preset entries, and dirty drafts disable config-affecting controls until Apply/Revert.
