...
| Module | Role |
|--------|------|
| `config_defaults.rs` | `ConfigDefaults`, `TurtleDefaults`, `ColorDefaults`, `LineColorDefaults`, `DefaultLineColorMode`, `GradientDefaults`, `HueCycleDefaults` — parsed from embedded `defaults.toml` via `ConfigDefaults::embedded()` (cached via `OnceLock`). `ParseConfigError` — error type covering TOML parse errors (`TomlParse`, `TomlDeserialize`) and domain validation failures (`Validation(ConfigError)`). Shared helpers `validate_step` and `validate_initial_heading` used by both defaults and editor-config paths. |
