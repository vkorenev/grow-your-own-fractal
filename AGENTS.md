...
- **Session config workspace**: `ConfigWorkspace` is the shared source of config editor state for both UIs. Each entry retains draft text independently from the last-applied document/config, Copy creates a custom copy using the selected valid draft or falling back to last-applied text, failed Apply keeps the current rendered scene, Revert restores the last-applied text, Reset restores bundled defaults for preset entries when the last-applied text differs from the default, custom entries have no bundled default, and dirty drafts disable config-affecting controls until Apply/Revert.
