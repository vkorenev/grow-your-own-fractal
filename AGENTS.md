...
| Module | Role |
|--------|------|
| `config.rs` | Two-phase config pipeline: `ConfigSource` wraps a `toml_edit::DocumentMut` (format-preserving, parse-only); `TryFrom<ConfigSource> for ConfigDocument` validates and produces a `ConfigDocument` that pairs the source with its validated `Config`; `From<ConfigDocument> for Config` extracts ownership. Validates symbols, rules, step/angle finiteness, bracket balance, optional background color, externally tagged line colors (`solid`, `gradient`, `hue_cycle`) including `gradient.topological_depth`, and hex color strings, then stores colors as validated `Rgb` values |
