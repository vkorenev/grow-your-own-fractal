...
| `color.rs` | `LineColorMode` — unified enum with `ALL`, `Display`, `from_key`/`key`, `from_line_color`, `default_line_color`; `ColorControlMemory` — per-mode `HexColor` slot store for UI color picker memory |
