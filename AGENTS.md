...
| `offscreen.rs` | Private module (under the `png` feature) with GPU plumbing shared by PNG and APNG export: `ExportScene` (one enum-backed type pairing the 2D or 3D line pipeline with bounds of the matching dimensionality, plus resolved `ColorParams`; exposes `height_for_width`, `write_camera`, `write_color_params`) and `RenderTarget` (offscreen texture + readback buffer + `ReadbackLayout`; `render_frame` returns tightly packed RGBA) |
| `png_export.rs` | Public PNG export surface; gated behind the `png` Cargo feature. `render_png` (caller-supplied device) / `render_png_standalone` (creates its own device) → `PngExport { width, height, bytes }`; shared `ExportError` used by both PNG and APNG paths; public `MIN_WIDTH`/`MAX_DIMENSION` width limits consumed by both UIs |
