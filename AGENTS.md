...
| Module | Role |
|--------|------|
| `config.rs` | Two-phase config pipeline: `ConfigSource` wraps a `toml_edit::DocumentMut` (format-preserving, parse-only); `TryFrom<ConfigSource> for ConfigDocument` validates and produces a `ConfigDocument` that pairs the source with its validated `Config`; `From<ConfigDocument> for Config` extracts ownership. Validates symbols, rules, step/angle finiteness, bracket balance, and per-component RGB color ranges |
| `config_workspace.rs` | Shared session config workspace: tracks optional draft TOML, a last-applied `ConfigDocument`, and an optional bundled default document per entry; exposes indexed copy/apply/revert/reset operations while each UI owns selection. Invalid presets are skipped with a warning at load time |
