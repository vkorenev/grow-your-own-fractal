...
| `line_renderer.rs` | `Segment2D`/`Segment3D` GPU instance types plus `TopologicalDepthSegment2D`/`TopologicalDepthSegment3D` for depth-aware rendering. `GrowableVertexBuffer` — grows to next power-of-two capacity on demand and counts segment instances. `LinePipeline2D` — 2D `Transform` uniform + `LinePipeline3D` — `Mvp` (64-byte MVP matrix) uniform; both own normal and topological-depth render pipelines and share a private `draw_line_list()` helper using `draw(0..2, 0..segment_count)`. `GpuContext`, `GpuInitError`, `FrameOutcome`, `SurfaceFrame`. Dimension and line-color-aware segment-count caps keep normal and depth segment buffers within wgpu's guaranteed 256 MiB vertex-buffer limit. |
